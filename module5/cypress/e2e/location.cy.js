...
describe("share location", () => {
  beforeEach(() => {
    cy.clock(); //manipulating the time
  ...
  it("should share a location URL", () => {
    cy.get('[data-cy="name-input"]').type("Donna Noble");
    cy.get('[data-cy="get-loc-btn"]').click();
    ...
    cy.get("@userLocation").then((fakePosition) => {
      ...
      cy.get("@saveToClipboard").should(
        ...
        new RegExp(`${latitude}.*${longitude}.*${encodeURI("Donna Noble")}`)
        );
    ...
    cy.tick(2000); // advancing the time
